...
namespace hltypes
{
	...
	public:
		...
		HL_INLINE Deque() : stddeque()
		{
		}
		/// @brief Copy constructor.
		/// @param[in] other Deque to copy.
		HL_INLINE Deque(const Deque<T>& other) : stddeque(other)
		{
		}
		/// @brief Destructor.
		HL_INLINE ~Deque()
		...
		HL_INLINE Deque<T> operator()(const int start, const int count) const
		{
			Deque<T> result;
			if (count > 0)
			{
		...
		/// @see equals
		HL_INLINE bool operator==(const Deque<T>& other) const
		{
		...
		HL_INLINE bool equals(const Deque<T>& other) const
		{
		...
		HL_INLINE bool nequals(const Deque<T>& other) const
		{
			if (this->size() != other.size())
			{
				return true;
		...
		/// @param[in] element Element to search for.
		/// @return Index of the given element or -1 if element could not be found.
		HL_INLINE Deque<int> indexes_of(T element) const
		{
			Deque<int> result;
			for_iter (i, 0, this->size())
			{
		...
		HL_INLINE bool contains(const Deque<T>& other) const
		{
			int index;
		...
		HL_INLINE void insert_at(const int index, const Deque<T>& other)
		{
			if (index > this->size())
			{
				throw container_index_error(index);
		...
		HL_INLINE void insert_at(const int index, const Deque<T>& other, const int count)
		{
			if (index > this->size())
			{
				throw container_index_error(index);
		...
		/// @param[in] count Number of elements to insert.
		HL_INLINE void insert_at(const int index, const Deque<T>& other, const int start, const int count)
		{
			if (index > this->size())
			{
				throw container_index_error(index);
		...
		HL_INLINE Deque<T> remove_at(const int index, const int count)
		{
		...
		HL_INLINE void remove(const Deque<T>& other)
		{
			int index;
			for_iter (i, 0, other.size())
			{
		...
		/// @param[in] other Deque of elements to remove.
		/// @return Number of elements removed.
		HL_INLINE int remove_all(const Deque<T>& other)
		{
			Deque<int> indexes;
			iterator_t it;
			int count = 0;
		...
		HL_INLINE void push_back(const Deque<T>& other)
		{
		...
		HL_INLINE void push_back(const Deque<T>& other, const int count)
		{
		...
		/// @param[in] count Number of elements to add.
		HL_INLINE void push_back(const Deque<T>& other, const int start, const int count)
		{
		...
		/// @param[in] other Deque of elements to add.
		HL_INLINE void push_front(const Deque<T>& other)
		{
		...
		/// @param[in] other Deque of elements to add.
		/// @param[in] count Number of elements to add.
		HL_INLINE void push_front(const Deque<T>& other, const int count)
		{
		...
		/// @param[in] start Start index of the elements to add.
		/// @param[in] count Number of elements to add.
		HL_INLINE void push_front(const Deque<T>& other, const int start, const int count)
		{
		...
		/// @return Deque of all removed elements.
		/// @note Elements in the returned Deque are in the same order as in the orignal Deque.
		HL_INLINE Deque<T> pop_front(const int count)
		{
			if (count > this->size())
			{
				throw container_range_error(0, count);
		...
		/// @return Deque of all removed elements.
		/// @note Elements in the returned Deque are in the same order as in the orignal Deque.
		HL_INLINE Deque<T> pop_back(const int count)
		{
			if (count > this->size())
			{
				throw container_range_error(0, count);
		...
		HL_INLINE void unite(const Deque<T>& other)
		{
			this->insert_at(this->size(), other);
			this->remove_duplicates();
		}
		...
		/// @param[in] element Element to unite with.
		/// @return A new Deque.
		HL_INLINE Deque<T> united(const T& element) const
		{
			Deque<T> result(*this);
		...
		/// @param[in] other Deque to unite with.
		/// @return A new Deque.
		HL_INLINE Deque<T> united(const Deque<T>& other) const
		{
			Deque<T> result(*this);
			result.unite(other);
			return result;
		...
		HL_INLINE void intersect(const Deque<T>& other)
		{
			Deque<T> result;
			for_iter (i, 0, this->size())
			{
		...
		/// @param[in] other Deque to intersect with.
		/// @return A new Deque.
		HL_INLINE Deque<T> intersected(const Deque<T>& other) const
		{
			Deque<T> result(*this);
		...
		HL_INLINE void differentiate(const Deque<T>& other)
		{
			int index;
			for_iter (i, 0, other.size())
			{
		...
		/// @return A new Deque.
		/// @note Unlike remove, this method ignores if the element is not in this Deque.
		HL_INLINE Deque<T> differentiated(const T& element) const
		{
			Deque<T> result(*this);
		...
		/// @return A new Deque.
		/// @note Unlike remove, this method ignore elements of other Deque that are not in this one.
		HL_INLINE Deque<T> differentiated(const Deque<T>& other) const
		{
			Deque<T> result(*this);
		...
		/// @return A new Deque.
		HL_INLINE Deque<T> reversed() const
		{
			Deque<T> result(*this);
			result.reverse();
			return result;
		...
		/// @return A new Deque.
		HL_INLINE Deque<T> removed_duplicates() const
		{
			Deque<T> result(*this);
		...
		/// @return A new Deque.
		/// @note The sorting order is ascending.
		HL_INLINE Deque<T> sorted() const
		{
			Deque<T> result(*this);
			result.sort();
			return result;
		...
		/// @note The sorting order is ascending.
		/// @note compare_function should return true if first element is less than the second element.
		HL_INLINE Deque<T> sorted(bool (*compare_function)(T, T)) const
		{
			Deque<T> result(*this);
		...
		/// @return A new Deque.
		HL_INLINE Deque<T> randomized() const
		{
			Deque<T> result(*this);
			result.randomize();
			return result;
		...
		HL_INLINE Deque<T> random(int count, bool unique = false) const
		{
			Deque<T> result;
			if (!unique)
			{
		...
		/// @param[in] unique Whether to force all random values to be unique.
		/// @return Deque of random elements selected from this one.
		HL_INLINE Deque<T> pop_random(int count, bool unique = false)
		{
			Deque<T> result;
			if (!unique)
			{
		...
		HL_INLINE Deque<T> find_all(bool (*condition_function)(T))
		{
			Deque<T> result;
			for_iter (i, 0, this->size())
			{
		...
		template <class S>
		HL_INLINE Deque<S> dyn_cast(bool include_nulls = false)
		{
			Deque<S> result;
			S value;
			for_iter (i, 0, this->size())
		...
		HL_INLINE bool includes(const Deque<T>& other) const
		{
		...
		/// @brief Same as contains.
		/// @see contains(const Deque<T>& other)
		HL_INLINE bool has(const Deque<T>& other) const
		{
			return this->contains(other);
		}
		/// @brief Same as contains.
		...
		/// @brief Same as contains.
		/// @see contains(const Deque<T>& other)
		HL_INLINE bool has_element(const Deque<T>& other) const
		{
			return this->contains(other);
		}
		/// @brief Same as contains.
		...
		HL_INLINE void add(const Deque<T>& other)
		{
			this->push_back(other);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other, const int count)
		HL_INLINE void add(const Deque<T>& other, const int count)
		{
			this->push_back(other, count);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other, const int start, const int count)
		HL_INLINE void add(const Deque<T>& other, const int start, const int count)
		{
		...
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other)
		HL_INLINE void append(const Deque<T>& other)
		{
			this->push_back(other);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other, const int count)
		HL_INLINE void append(const Deque<T>& other, const int count)
		{
			this->push_back(other, count);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other, const int start, const int count)
		HL_INLINE void append(const Deque<T>& other, const int start, const int count)
		{
			this->push_back(other, start, count);
		}
		/// @brief Same as push_back.
		...
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other).
		HL_INLINE void push_last(const Deque<T>& other)
		{
			this->push_back(other);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other, const int count).
		HL_INLINE void push_last(const Deque<T>& other, const int count)
		{
			this->push_back(other, count);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other, const int start, const int count).
		HL_INLINE void push_last(const Deque<T>& other, const int start, const int count)
		{
			this->push_back(other, start, count);
		}
		/// @brief Same as push_back.
		...
		HL_INLINE void push_first(const Deque<T>& other)
		{
			this->push_front(other);
		}
		/// @brief Same as push_front.
		/// @see push_front(const Deque<T>& other, const int count).
		HL_INLINE void push_first(const Deque<T>& other, const int count)
		{
			this->push_front(other, count);
		}
		/// @brief Same as push_front.
		/// @see push_front(const Deque<T>& other, const int start, const int count).
		HL_INLINE void push_first(const Deque<T>& other, const int start, const int count)
		{
		...
		HL_INLINE Deque<T> pop_first(const int count)
		{
		...
		HL_INLINE Deque<T> pop_last(const int count)
		{
		...
		/// @brief Same as pop_front.
		/// @see pop_front(const int count).
		HL_INLINE Deque<T> remove_front(const int count)
		{
			return this->pop_front(count);
		}
		/// @brief Same as pop_back.
		...
		/// @brief Same as pop_back.
		/// @see pop_back(const int count).
		HL_INLINE Deque<T> remove_back(const int count)
		{
			return this->pop_back(count);
		}
		/// @brief Same as pop_front.
		...
		/// @brief Same as pop_front.
		/// @see pop_front(const int count).
		HL_INLINE Deque<T> remove_first(const int count)
		{
			return this->pop_front(count);
		}
		/// @brief Same as pop_back.
		...
		/// @brief Same as pop_back.
		/// @see pop_back(const int count).
		HL_INLINE Deque<T> remove_last(const int count)
		{
			return this->pop_back(count);
		}
		...
		HL_INLINE Deque<T> remove_random(const int count)
		{
		...
		HL_INLINE Deque<T> pop(const int index, const int count)
		{
		...
		/// @brief Same as remove_at.
		/// @see remove_at(const int index, const int count)
		HL_INLINE Deque<T> pop_at(const int index, const int count)
		{
			return this->remove_at(index, count);
		}
		...
		HL_INLINE int pop_all(const Deque<T>& other)
		{
			return this->remove_all(other);
		}
		/// @brief Same as push_back.
		/// @see push_back(const T& element)
		HL_INLINE Deque<T>& operator<<(const T& element)
		{
			this->push_back(element);
			return (*this);
		}
		/// @brief Same as push_back.
		/// @see push_back(const Deque<T>& other)
		HL_INLINE Deque<T>& operator<<(const Deque<T>& other)
		{
			this->push_back(other);
			return (*this);
		}
		...
		HL_INLINE Deque<T>& operator-=(T element)
		{
			this->remove(element);
			return (*this);
		}
		...
		HL_INLINE Deque<T> operator+(const T& element) const
		{
			Deque<T> result(*this);
			result += element;
			return result;
		...
		HL_INLINE Deque<T> operator+(const Deque<T>& other) const
		{
			Deque<T> result(*this);
			result += other;
			return result;
		...
		HL_INLINE Deque<T> operator-(T element) const
		{
			Deque<T> result(*this);
			result -= element;
			return result;
