...
IF ERRORLEVEL 1 (
	...
	IF NOT EXIST "%MsBuildCppDir%\MsBuild\Android" (
		IF NOT EXIST "%MsBuildCppDir%\MsBuild\Android-x86" (
			goto start
		)
	)
	echo.An HL-Android MSBuild toolset already exists.
	echo.Continuing will delete the version already installed.
	rd "%MsBuildCppDir%\MsBuild\Android" /s
	IF EXIST "%MsBuildCppDir%\MsBuild\Android" (
		goto error
	)
	rd "%MsBuildCppDir%\MsBuild\Android-x86" /s /q
	IF EXIST "%MsBuildCppDir%\MsBuild\Android-x86" (
		goto error
	)
